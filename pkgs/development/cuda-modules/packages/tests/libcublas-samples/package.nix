...
let
  ...
  emulationHelperMissing = sampleRoot: {
    ...
    # nvcc's own words, so that the day upstream defines the function -- or renames the call -- this
    # sample comes back into the suite instead of staying out on a note nobody rereads. 2 rather
    # than 1: the sample configures perfectly well and it is `make` that reports the compilation
    # error, which is itself part of what is being claimed here.
    problemEvidence.cublasEmulationHelperUndefined = {
