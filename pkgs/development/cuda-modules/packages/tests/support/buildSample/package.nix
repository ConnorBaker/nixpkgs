...
}:
{
  ...
  # required to match. An opt-out which goes quiet is worth nothing: the skip for host-only code
  # below is written against `cuobjdump`'s own statement rather than against an empty result for
  # exactly that reason, since dropping `cuda_cuobjdump` from `nativeBuildInputs` is enough to make
  # every binary in the tree look like it holds no device code.
  ...
  # inputProblems :: AttrsOf Problem
  # Reasons a package this sample is built against is unavailable here, collected by `mkSamples` and
  # named after the package they came from. Distinct from the sample's own `meta.problems`, and
  # deliberately so: `certifyPrograms` below drops the sample's problems and keeps these, because a
  # project which fails to compile can still be configured while one whose component is missing
  # cannot. They are merged into `meta.problems` for the sample itself, where the distinction does
  # not matter -- either way it cannot be built.
  inputProblems ? { },

  # problemEvidence :: AttrsOf { expectedBuilderExitCode :: Int; expectedBuilderLogEntries :: [String]; }
  # For a problem recorded about this sample in `meta.problems`, what a build of it must still do to
  # show that the problem is still there. `mkProblemExpiryChecks` turns each entry into a derivation
  # which lifts the guard, builds the sample and requires the failure; the note on that file explains
  # why a recorded problem needs one at all, and why the failure must be the recorded failure rather
  # than any failure.
  #
  # Declared beside the `meta.problems` entry it belongs to, in the component's package.nix, because
  # the evidence and the message are the same measurement written twice -- the message quotes the
  # compiler, and this is that quotation in a form a build can be held to. `meta.problems` cannot
  # carry it: an entry admits `kind`, `message` and `urls` and nothing else, and stdenv checks that.
  #
  # Both fields are stated at every site rather than defaulted, because the pair of them is the
  # claim. The exit code is not incidental detail: 1 is CMake refusing to configure and 2 is make
  # reporting a compilation error, and which of the two a sample produces is part of what several of
  # these problems say about it.
  #
  # Only a sample's own problems can be evidenced. A requirement it does not meet, or a package it is
  # built against being unavailable, is not something to lift and rebuild -- there is no build to
  # attempt -- so the assertion below refuses a key naming one.
  problemEvidence ? { },
...
let
  ...
  # Everything said about this sample being unavailable here, from all three sources: the
  # requirements above, the packages it is built against, and whatever the component's package.nix
  # measured. Ordered so that a problem stated about this sample wins over one inherited from an
  # input, which in turn wins over a requirement: the more specific the statement, the later it is
  # applied.
  #
  # `certifyPrograms` needs to tell these apart rather than take the whole set, because whether a
  # project which will not build can still be *configured* is exactly what decides how its program
  # list can be checked; see `certifiableProblems` below.
  allProblems = problems // inputProblems // meta.problems or { };

  # A key naming no problem of this sample's own. Both ways of getting one are silent otherwise: a
  # misspelt name produces a check which lifts nothing and asserts a failure the sample was never
  # said to have, and a name which used to be a problem and is not one any more produces a check
  # whose subject has already been deleted -- which is precisely the moment the evidence beside it
  # should have gone too.
  unevidenceableProblems = lib.subtractLists (lib.attrNames (meta.problems or { })) (
    lib.attrNames problemEvidence
  );

  # Every evidenced problem is lifted for every one of these checks -- `mkProblemExpiryChecks` says
  # why -- so they all describe one build, and a build stops once. Two problems recorded against the
  # same sample can therefore only both be demonstrated if they surface together, which means the
  # same exit status: 1 is CMake refusing to configure and 2 is make reporting a compilation error,
  # and a sample cannot do both. Stating two would leave one check asserting an outcome the other
  # rules out, and it would fail as a mismatched log rather than as the contradiction it is.
  evidencedExitCodes = lib.unique (
    lib.mapAttrsToList (_: evidence: evidence.expectedBuilderExitCode) problemEvidence
  );
in
assert lib.assertMsg (unevidenceableProblems == [ ]) (
  "sample ${sampleRoot}: problemEvidence names"
  + " ${lib.concatStringsSep ", " unevidenceableProblems}, which is not among the problems this"
  + " sample records about itself; a requirement it does not meet or an unavailable input is not"
  + " evidenced by building it, because there is no build to attempt"
);
assert lib.assertMsg (lib.length evidencedExitCodes <= 1) (
  "sample ${sampleRoot}: problemEvidence expects builder exit codes"
  + " ${lib.concatMapStringsSep ", " toString (lib.sort lib.lessThan evidencedExitCodes)}, and every"
  + " one of these problems is lifted for every check, so they all describe the same build -- which"
  + " can only end one way; record the problems which surface together, or evidence one of them and"
  + " say in the other's message why it cannot be shown while the first holds"
);
backendStdenv.mkDerivation (
  ...
  // {
    ...
    passthru = {
      # One derivation per evidenced problem, each of which lifts the guards and requires this build
      # to fail in the way that problem describes. Empty for the overwhelming majority of samples,
      # which have nothing recorded against them.
      problemsStillHold = mkProblemExpiryChecks {
        sample = finalAttrs.finalPackage;
        inherit problemEvidence;
      };
      ...
      #
      # This configures and stops; it compiles nothing and needs no GPU. The sample's own problems
      # are therefore dropped, so that a sample which cannot be *built* here is still asked about,
      # which is the entire point: nvCOMP's two configure perfectly well and only fail to compile.
      #
      # `inputProblems` are kept, and the distinction is not a nicety. Dropping a problem says "this
      # derivation can still be built"; that is true of a sample whose sources will not compile, and
      # false of one whose component is unavailable, because configuring needs the component in
      # `buildInputs` just as compiling does. Clearing both together left this reporting itself
      # available with an input that refuses to evaluate, and `mkSamples` aggregating it into a check
