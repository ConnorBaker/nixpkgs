...
# `meta.mainProgram` -- `writeTextFile` derives it from the `/bin/<name>` destination
# `writeShellApplication` gives it, so nothing here has to set it -- which is what lets `mkTest`
# derive a sandboxed run from it without duplicating the invocation, and what lets a user run the
# same test outside the sandbox under nixGL.
#
...
let
  ...
  testerName = "${cudaNamePrefix}-${component.pname}-tester-${name}";
  ...
  # there is nothing for bash or for shellcheck to infer. `excludeShellChecks = [ "SC2054" ]` would
  # also silence it, and is not used: it would suppress the warning wherever it is right as well.
  ...
  stageDataFiles = lib.concatMapAttrsStringSep "\n" (path: source: ''
    ...
    mkdir --parents "$(dirname "$workRoot/${path}")"
...
writeShellApplication {
  name = testerName;
  # Stated rather than left to be parsed back out of `name`. `lib.getName` is `pname` if there is
  # one and `builtins.parseDrvName name` otherwise, and that parse ends the name at the first dash
  # followed by a non-letter -- which every project path with a numbered segment has, so
  # `cuBLAS/Level-1/amax` and its fifty-two siblings all reduced to one name. That name is what
  # `config.problems.handlers.<package>` is keyed by, so a user wanting to ignore one sample's
  # problem could not name it: the key they had to write covered every sample under `Level-1`.
  derivationArgs = {
    pname = testerName;
    inherit (component) version;
  };
  inherit runtimeEnv;
  # `writeShellApplication` defaults this to true, which appends the caller's PATH to the one it
  # builds from `runtimeInputs`. Everything this script runs is a bash builtin, coreutils, or the
  # program under test, so inheriting adds nothing except the chance that a run outside the sandbox
  # resolves against the host -- and an entry missing from `runtimeInputs` would then be masked here
  # rather than reported.
  inheritPath = false;
  runtimeInputs = [
    sample
    # The staging and the output check below are coreutils.
    coreutils
  ]
  ++ runtimeInputs;
  text = ''
  ...
  passthru = {
    ...
    # The compiled sample this runs, reachable for building or debugging without being an attribute
    # of the component's `tests` -- where it would be neither a tester nor a check, and would be
    # built twice under two names by anything walking the set.
    inherit sample;
  };
