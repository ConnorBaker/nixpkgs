...
#
# Reached as the tester's own `passthru.gpuCheck` rather than as an attribute of the component's
# tests, which is what keeps a job needing hardware out of the set CI builds from a `<component>:`
# commit; `mkSamples` attaches it and says why.
#
...
let
  # Derived from the tester's `pname` rather than its `name`, so that this carries one too. Without
  # it `lib.getName` parses the name instead, and stops at the first dash followed by a non-letter --
  # which is any project path with a numbered segment, so every test under `cuBLAS/Level-1` answered
  # to the same name. See the note beside `mkTester`'s own `pname`.
  testName =
    if lib.hasInfix "-tester-" tester.pname then
      lib.replaceString "-tester-" "-test-" tester.pname
    else
      "${tester.pname}-test";
in
runCommand testName
  {
    ...
    # The tester -- and through its own passthru the compiled sample -- so that a failing run can be
    # reproduced by hand, outside the sandbox and under nixGL, starting from whichever of the two
    # names the failure was reported under. This is the same store path as the attribute this check
    # was reached through, since attaching the check changed only a `passthru`.
    passthru = {
      inherit tester;
    };
