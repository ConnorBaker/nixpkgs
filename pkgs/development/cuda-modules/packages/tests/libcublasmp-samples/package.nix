...
mkSamples {
  ...
  sampleArgsFor = _: {
    ...
    # nvcc's own words, so that the day cuBLASMp declares the function -- or upstream stops calling
    # it -- these nine executables come back into the suite rather than staying out on a note nobody
    # rereads. 2 rather than 1: the project configures, and it is `make` that reports the
    # compilation error, which is what says the blocker is the component's headers and not its
    # CMake package config.
    problemEvidence.cublasmpMatmulDescriptorApiMissing = {
