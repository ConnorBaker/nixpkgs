...
let
  # The checkout these samples are built from, named once and handed to `mkSamples` below, because
  # the paths this file puts on a command line have to come out of the same one. Reaching for
  # `cuda-library-samples-src` at each of those would name whichever revision is the default rather
  # than the one this component selected, and the two agree only until a component pins back the way
  # `libcublas-samples` does. `mkSamples` refuses a store path from anywhere else.
  ...
  inputImagesOf = project: "${src}/nvJPEG/${project}/input_images/";
  ...
  # satisfies. Found by the `programs` check, which configures each project and so sees a
  # requirement that stops the configure; nothing else here had looked, and the sample was claimed
  # available on 12.6 and 12.8 while being unbuildable on both.
  ...
  testArgs = {
    ...
    "nvJPEG/Image-Resize-WaterMark".imageResizeWatermark = resizeArgs "Image-Resize-WaterMark" // {
      ...
      dataFiles."NVLogo.jpg" = "nvJPEG/Image-Resize-WaterMark/NVLogo.jpg";
    };
  };
in
mkSamples {
