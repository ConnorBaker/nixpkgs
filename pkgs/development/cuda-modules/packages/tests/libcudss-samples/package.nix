...
let
  ...
  requiresCudss070 = project: {
    ...
    # CMake's own refusal, so that the day a package set ships cuDSS 0.7.0 these projects come back
    # into the suite rather than staying out on a note nobody rereads. 1 rather than 2, and the
    # difference is the claim: this sample never reaches a compiler, which is why nothing about the
    # toolkit or the sources can make it build.
