...
let
  ...
  # the files this file puts on a command line have to come out of the same one. Reaching for
  # `cuda-library-samples-src` at each of those would name whichever revision is the default rather
  # than the one this component selected; `mkSamples` refuses a store path from anywhere else.
  ...
  tiffOf = project: "${src}/nvTIFF/${project}/images/bali_notiles.tif";
