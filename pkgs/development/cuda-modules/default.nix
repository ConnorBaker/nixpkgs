...
let
  ...
  cudaPackagesFixedPoint =
    ...
    // (
      ...
      in
      # The `tests` `packagesFromDirectoryRecursive` produced is dropped rather than overwritten by
      # the one below. Both spellings give the same set, but `//` says it silently: a reader cannot
      # see that the left-hand side has a `tests` at all, and neither can a reviewer of a change
      # which stops the right-hand side from providing one. `removeAttrs` makes the discard the
      # thing being read.
