...
- `symlinkJoin` no longer uses `lndir` to merge its `paths`; the union is now implemented directly
  in bash. Different files at the same relative path now fail the build by default instead of
  silently keeping the first input. Byte-identical regular files remain a no-op. A collision can be
  resolved with the new `pathStrategies`/`strategies` arguments; built-in defaults concatenate the
  standard `nix-support` propagation and setup-hook files, fixing their previous truncation when
  multiple joined derivations propagated inputs.
